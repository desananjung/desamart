...
router.post('/members', authorize('ADMIN'), async (req, res, next) => {
  try {
    ...
    if (!cooperativeId || !userId) return badRequest(res, 'Cooperative ID dan User ID wajib diisi');
...
router.get('/members', async (req, res, next) => {
  try {
    ...
    if (!cooperativeId) return badRequest(res, 'Cooperative ID wajib diisi');
...
router.post('/loans/:loanId/pay', async (req, res, next) => {
  try {
    ...
    if (!amount) return badRequest(res, 'Amount wajib diisi');
